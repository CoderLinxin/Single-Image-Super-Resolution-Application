...
def main(img_path: str, device: torch.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
    ...
    with torch.no_grad():
        ...
        # 加载模型权重参数
        load_model_weights('weights/hitsir_pro测试浅层特征提取2_loss(l1)_mulsizeconvextract(True)_casa(True)_fusion_embed_dim(60)_len(depths)(4)_augment/new_epoch_model.pth', model)
        # 获取 lr 变换
        ...
        # 计算 psnr 指标(rgb)
        hr = lr_transform(hr).to(device).unsqueeze(0)
        r_psnr = peak_signal_noise_ratio(hr.cpu().numpy()[0, 0, :, :], sr.cpu().numpy()[0, 0, :, :], data_range=1.)
        g_psnr = peak_signal_noise_ratio(hr.cpu().numpy()[0, 1, :, :], sr.cpu().numpy()[0, 1, :, :], data_range=1.)
        b_psnr = peak_signal_noise_ratio(hr.cpu().numpy()[0, 2, :, :], sr.cpu().numpy()[0, 2, :, :], data_range=1.)
        print(f"psnr: {(r_psnr + b_psnr + g_psnr) / 3.}")
        # sr 变换为 PIL Image
