...
def hitsir_pro测试浅层特征提取3_experiment(
        is_test: bool,
        loss: str,
    ...
    # 获取消融实验保存结果文件夹名称
    folder_name = f'hitsir_pro测试浅层特征提取3_loss({loss})_mulsizeconvextract({is_mult_size_conv_feat_extract})_casa({is_channel_spatial_attn}){"_fusion" if is_fusion else ""}_embed_dim({embed_dim})_len(depths)({len(depths)})'
    if is_augment:
