...
class HITSIRPRO测试浅层特征提取3Experiment(Experiment):
    def __init__(self, **kwargs):
        super(HITSIRPRO测试浅层特征提取3Experiment, self).__init__(**kwargs)
    ...
    def init_model(self):
        ...
        super(HITSIRPRO测试浅层特征提取3Experiment, self).init_model()


def hitsir_pro测试浅层特征提取3_测试经典超分_experiment(
        is_test: bool,
        loss: str,
    ...
    train_data_config = DatasetConfig(
        ...
        is_bsrgan_degrade=is_bsrgan_degrade
    )
    ...
    # 获取消融实验保存结果文件夹名称
    folder_name = f'hitsir_pro测试浅层特征提取3_测试经典超分_loss({loss})_mulsizeconvextract({is_mult_size_conv_feat_extract})_casa({is_channel_spatial_attn}){"_fusion" if is_fusion else ""}_embed_dim({embed_dim})_len(depths)({len(depths)})'
    if is_augment:
    ...
    model_config = HITModelConfig(
        ...
        train_data_name_list=[
            # 基本训练集
            'DIV2K_train_HR'
            # 'blend', 'RealSR(V3)', , 'wuthering_wave', 'Flickr2K_HR', 'face', 'REDS2K',
        ...
        # test_data_name_list=['BSD100', 'Set5', 'Set14', 'Urban100', 'Manga109'],
        test_data_name_list=['Set14'],
    ...
    # 创建实验
    experiment = HITSIRPRO测试浅层特征提取3Experiment(
