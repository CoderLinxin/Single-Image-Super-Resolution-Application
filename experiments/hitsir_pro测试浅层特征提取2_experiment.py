...
class HITSIRPRO测试浅层特征提取2Experiment(Experiment):
    def __init__(self, **kwargs):
        super(HITSIRPRO测试浅层特征提取2Experiment, self).__init__(**kwargs)
    ...
    def init_model(self):
        ...
        super(HITSIRPRO测试浅层特征提取2Experiment, self).init_model()


def hitsir_pro测试浅层特征提取2_experiment(
        is_test: bool,
        loss: str,
    ...
    # 获取消融实验保存结果文件夹名称
    folder_name = f'hitsir_pro测试浅层特征提取2_loss({loss})_mulsizeconvextract({is_mult_size_conv_feat_extract})_casa({is_channel_spatial_attn}){"_fusion" if is_fusion else ""}_embed_dim({embed_dim})_len(depths)({len(depths)})'
    if is_augment:
    ...
    # 创建实验
    experiment = HITSIRPRO测试浅层特征提取2Experiment(
