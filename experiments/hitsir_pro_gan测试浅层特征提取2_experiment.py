...
class HITSIRPROGAN测试浅层特征提取2Experiment(Experiment):
    def __init__(self, **kwargs):
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).__init__(**kwargs)
    ...
    def init_model(self):
        ...
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).init_model()

    def init_tools(self):
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).init_tools()
        self.epoch_discriminator_loss = AverageMeter()

    def train(self):
        self.epoch_discriminator_loss.reset()
        self.discriminator.train()
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).train()

    def eval(self, start_epoch=None):
        self.discriminator.eval()
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).eval(start_epoch)

    def init_optimizer_loss_function(self, params=None):
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).init_optimizer_loss_function()
    ...
    def load_model_weights_scheduler(self, is_gan_start: bool = False):
        ...
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).load_model_weights_scheduler(is_gan_start=self.start_epoch == 1)
    ...
    def load_log(self):
        ...
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).load_log()
    ...
    def train_dataloader_process(
        ...
        super(HITSIRPROGAN测试浅层特征提取2Experiment, self).train_dataloader_process(is_end, _)
...
def hitsir_pro_gan测试浅层特征提取2_experiment(
        is_test: bool,
        loss: str,
    ...
    # 获取消融实验保存结果文件夹名称
    folder_name = f'hitsir_pro_gan测试浅层特征提取2_loss({loss})_mulsizeconvextract({is_mult_size_conv_feat_extract})_casa({is_channel_spatial_attn}){"_fusion" if is_fusion else ""}_embed_dim({embed_dim})_len(depths)({len(depths)})'
    if is_augment:
    ...
    # 创建实验
    experiment = HITSIRPROGAN测试浅层特征提取2Experiment(
