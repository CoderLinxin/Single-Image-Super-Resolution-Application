...
class MultipleSizeConvExtract(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 64):
        ...
        self.attn1 = nn.Conv2d(out_channels, out_channels, 1, 1, 0)
        self.attn2 = nn.Conv2d(out_channels, out_channels, 1, 1, 0)
        self.attn3 = nn.Conv2d(out_channels, out_channels, 1, 1, 0)
        self.attn4 = nn.Conv2d(out_channels, out_channels, 1, 1, 0)
        self.attn5 = nn.Conv2d(out_channels, out_channels, 1, 1, 0)

        self.dot1 = nn.Conv2d(1, out_channels, 3, 1, 1)
        self.dot2 = nn.Conv2d(1, out_channels, 3, 1, 1)
        self.dot3 = nn.Conv2d(1, out_channels, 3, 1, 1)
        self.dot4 = nn.Conv2d(1, out_channels, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        x3_attn = self.attn1(x3)
        x5_attn = self.attn2(x5)
        x7_attn = self.attn3(x7)
        x9_attn = self.attn4(x9)
        x_attn = self.attn5(x)

        # 计算各个尺寸卷积核的特征图结果与y的点积得到注意力权重
        x3_attention = torch.sigmoid(self.dot1((x_attn * x3_attn).sum(dim=1).unsqueeze(1)))  # (b,c,h,w)
        x5_attention = torch.sigmoid(self.dot2((x3_attn * x5_attn).sum(dim=1).unsqueeze(1)))  # (b,c,h,w)
        x7_attention = torch.sigmoid(self.dot3((x5_attn * x7_attn).sum(dim=1).unsqueeze(1)))  # (b,c,h,w)
        x9_attention = torch.sigmoid(self.dot4((x7_attn * x9_attn).sum(dim=1).unsqueeze(1)))  # (b,c,h,w)
        ...
        return self.conv_last(torch.cat((x, x3, x5, x7, x9), dim=1)) * 0.2 + x3
