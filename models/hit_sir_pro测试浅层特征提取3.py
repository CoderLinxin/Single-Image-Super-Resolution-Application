...
class mydwconv(nn.Module):
    def __init__(self, hidden_features):
        ...
        self.depthwise_conv2 = nn.Sequential(
            nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=5 // 2, dilation=2,
                      groups=hidden_features), nn.GELU())
        self.depthwise_conv3 = nn.Sequential(
            nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=7 // 2, dilation=3,
                      groups=hidden_features), nn.GELU())
        self.hidden_features = hidden_features

    def forward(self, x, x_size):
        x = x.transpose(1, 2).view(x.shape[0], self.hidden_features, x_size[0], x_size[1]).contiguous()  # b Ph*Pw c
        x = self.depthwise_conv1(x) + self.depthwise_conv2(x) + self.depthwise_conv3(x)
        x = x.flatten(2).transpose(1, 2).contiguous()
        return x
...
class ConvFFN(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.act = act_layer()
        self.dwconv = mydwconv(hidden_features=hidden_features)
...
class MultipleSizeConvExtract(nn.Module):
    def __init__(self, in_channels: int = 3, out_channels: int = 64):
        ...
        self.attn1 = dwconv(out_channels)
        self.attn2 = dwconv(out_channels)
        self.attn3 = dwconv(out_channels)
        self.attn4 = dwconv(out_channels)
        self.attn5 = dwconv(out_channels)

        self.dot = nn.Conv2d(4, out_channels * 4, 3, 1, 1)
        self.group_norm = nn.GroupNorm(1, 1)

        #  用于调整特征的通道维度
        self.conv_last = nn.Conv2d(5 * out_channels, out_channels, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        x1_attn = self.attn1(x1.permute(0, 2, 3, 1).reshape(b, -1, self.out_channels), (h, w)).permute(0, 2, 1).reshape(b, self.out_channels, h, w)
        x3_attn = self.attn2(x3.permute(0, 2, 3, 1).reshape(b, -1, self.out_channels), (h, w)).permute(0, 2, 1).reshape(b, self.out_channels, h, w)
        x5_attn = self.attn3(x5.permute(0, 2, 3, 1).reshape(b, -1, self.out_channels), (h, w)).permute(0, 2, 1).reshape(b, self.out_channels, h, w)
        x7_attn = self.attn4(x7.permute(0, 2, 3, 1).reshape(b, -1, self.out_channels), (h, w)).permute(0, 2, 1).reshape(b, self.out_channels, h, w)
        x9_attn = self.attn5(x9.permute(0, 2, 3, 1).reshape(b, -1, self.out_channels), (h, w)).permute(0, 2, 1).reshape(b, self.out_channels, h, w)

        dot3 = self.group_norm((x3_attn * x1_attn).sum(dim=1).unsqueeze(1))
        dot5 = self.group_norm((x5_attn * x1_attn).sum(dim=1).unsqueeze(1))
        dot7 = self.group_norm((x7_attn * x1_attn).sum(dim=1).unsqueeze(1))
        dot9 = self.group_norm((x9_attn * x1_attn).sum(dim=1).unsqueeze(1))

        # 计算各个尺寸卷积核的特征图结果与y的点积得到注意力权重
        x3_attention, x5_attention, x7_attention, x9_attention = torch.chunk(torch.sigmoid(self.dot(torch.cat((
            dot3, dot5, dot7, dot9
        ), dim=1))), 4, dim=1)  # (b,c,h,w)

        # 加权(加了残差性能更好)
        x3_weight = x3 * x3_attention + x3  # (b,out_channels,h,w)
        x5_weight = x5 * x5_attention + x5  # (b,out_channels,h,w)
        x7_weight = x7 * x7_attention + x7  # (b,out_channels,h,w)
        x9_weight = x9 * x9_attention + x9  # (b,out_channels,h,w)

        return self.conv_last(torch.cat((x1_attn, x3_weight, x5_weight, x7_weight, x9_weight), dim=1)) * 0.25 + x3_weight
...
class UnionAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 对 H 维度计算最大池化、平均池化
        avg_out_h = torch.mean(x, dim=2, keepdim=True).permute(0, 2, 1, 3)  # (b,c,1,w)->(b,1,c,w)
        max_out_h = torch.max(x, dim=2, keepdim=True)[0].permute(0, 2, 1, 3)  # (b,c,1,w)->(b,1,c,w)
        h_attention = self.conv2(torch.cat((avg_out_h, max_out_h), dim=1)).permute(0, 2, 1, 3)  # (b,c,1,w)
        # 对 W 维度计算最大池化、平均池化
        avg_out_w = torch.mean(x, dim=3, keepdim=True).permute(0, 1, 3, 2).permute(0, 2, 1, 3)  # (b,c,h,1)->(b,c,1,h)->(b,1,c,h)
        max_out_w = torch.max(x, dim=3, keepdim=True)[0].permute(0, 1, 3, 2).permute(0, 2, 1, 3)  # (b,c,h,1)->(b,c,1,h)->(b,1,c,h)
        w_attention = self.conv3(torch.cat((avg_out_w, max_out_w), dim=1)).permute(0, 2, 1, 3).permute(0, 1, 3, 2)  # (b,c,h,1)
...
class SpatialChannelAttention(nn.Module):
    def __init__(self, dim):
        ...
        self.channel1 = nn.Conv2d(1, dim // 2, 3, 1, 1)
        self.channel2 = nn.Conv2d(1, dim // 2, 3, 1, 1)

        # 下面 linear 层是参数量多少的关键(为了节省参数量,做了一个通道压缩)
        self.linear1_first = nn.Linear(dim // 2, dim // 10)
        self.linear1_second = nn.Linear(dim // 10, dim // 2)
        self.linear2_first = nn.Linear(dim // 2, dim // 10)
        self.linear2_second = nn.Linear(dim // 10, dim // 2)
        # 池化操作非常费时间
    ...
    def forward(self, x):
        ...
        channel_avg = torch.mean(x1, dim=1, keepdim=True)  # (b,1,h,w)
        channel_max = torch.max(x1, dim=1, keepdim=True)[0]  # (b,1,h,w)
        channel_attn1 = self.channel1(channel_avg)  # (b,c//2,h,w)
        channel_attn2 = self.channel2(channel_max)  # (b,c//2,h,w)

        spatial_avg = self.avg(x2)  # (b,c//2,1,1)
        spatial_max = self.max(x2)  # (b,c//2,1,1)
        ...
        attn = torch.cat((channel_attn1 + spatial_attn1, channel_attn2 + spatial_attn2), dim=1)

        attn = self.norm(attn.permute(0, 2, 1, 3).permute(0, 1, 3, 2).reshape(b, h * w, c)).permute(0, 2, 1).reshape(b, c, h, w)

        return (attn + x) / 2.  # (b,c,h,w)
