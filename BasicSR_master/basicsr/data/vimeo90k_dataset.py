...
class Vimeo90KDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        with open(opt['meta_info_file'], 'r') as fin:
            self.keys = [line.split(' ')[0] for line in fin]  # 00001/0002
    ...
    def __getitem__(self, index):
        ...
        # get the neighboring LQ frames(以 Vimeo90K 数据集为例,每个样本对应的支持帧序列共7帧)
        img_lqs = []
    ...
    def __len__(self):
        # 对于 Vimeo90K 训练数据集来说,值为64612,每一个样本包含了7帧图片
...
class Vimeo90KRecurrentDataset(Vimeo90KDataset):
    """
    适用于循环神经网络结构的视频超分辨率训练集结构
    在这里 num_frame == 序列帧数量,一个序列仍然当成一个样本,只不过从输入一个序列(窗口)来预测中间帧变成了输入一个序列来预测整个序列
    """
    def __init__(self, opt):
