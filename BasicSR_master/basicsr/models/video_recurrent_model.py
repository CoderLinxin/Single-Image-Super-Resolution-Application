    ...
    def setup_optimizers(self):
        ...
        if flow_lr_mul == 1:
            ...
            optim_params = [
                ...
                {
                    # (预训练的)光流估计网络 spynet 也需要训练,只不过训练的学习率会再乘以一个系数(相当于迁移学习)
                    'params': flow_params,
                    'lr': train_opt['optim_g']['lr'] * flow_lr_mul  # 这里是设置初始学习率(最大学习率)
                },
            ]
    ...
    def optimize_parameters(self, current_iter):
        if self.fix_flow_iter:
            ...
            if current_iter == 1:
                ...
                # 在训练早期由于视频超分网络性能较低,故将EDVR的特征提取网络和spynet的光流估计网络的参数冻结
                for name, param in self.net_g.named_parameters():
                    if 'spynet' in name or 'edvr' in name:
                        param.requires_grad_(False)
                # 等到迭代次数等于fix_flow_iter时才将EDVR和spynet进行解冻
