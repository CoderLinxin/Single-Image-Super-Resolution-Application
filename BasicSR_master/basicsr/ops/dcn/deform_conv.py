...
class ModulatedDeformConvPack(ModulatedDeformConv):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conv_offset = nn.Conv2d(
            self.in_channels,
            # 通道数为 deformable_groups*3N,其中offset占2N, mask占1N, 其中 N=kernel_size[0]*kernel_size[1]
            self.deformable_groups * 3 * self.kernel_size[0] * self.kernel_size[1],
            kernel_size=self.kernel_size,
            stride=_pair(self.stride),  # (self.stride,self.stride)
            padding=_pair(self.padding),  # (self.padding,self.padding)
            dilation=_pair(self.dilation),  # (self.dilation,self.dilation)
            bias=True
        )
        self.init_weights()
